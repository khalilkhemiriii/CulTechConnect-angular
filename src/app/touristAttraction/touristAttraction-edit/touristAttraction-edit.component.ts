...
@Component({
  selector: 'app-touristAttraction-edit',
  templateUrl: './touristAttraction-edit.component.html',
  styleUrls: ['./touristAttraction-edit.component.css']
})
export class EditTouristAttractionComponent implements OnInit  {
  ...
  sub:any;
  constructor(private router: Router, private touristAttractionService: TouristAttractionService,private route: ActivatedRoute, private httpClient: HttpClient, private userService: UserService) { }
  ngOnInit() {
    ...
    this.sub = this.route.params.subscribe(params => {
      this.touristAttractionid = +params['id'];
      this.touristAttractionService.getAllTouristAttractionImagesById(this.touristAttractionid)
        .subscribe(data => {
          this.touristAttractionImages = data;
        });
    });
    this.sub = this.route.params.subscribe(params => {
      this.touristAttractionid = +params['id'];
      this.touristAttractionService.getTouristAttractionById(this.touristAttractionid)
        .subscribe(data => {
  ...
  editTouristAttraction() {
    this.touristAttractionService.editTouristAttraction(this.touristAttraction)
      .subscribe(data => this.router.navigate(['touristAttraction-page/'+this.touristAttractionid]));
  }
  ...
  onUpload() {
    ...
    for (let i = 0; i < filesAmount; i++) {
      ...
      this.httpClient.post('http://localhost:8083/touristAttraction/upload/' + this.touristAttractionid, uploadData)
        .subscribe(
          res => {
            console.log(res);
  ...
  deleteTouristAttractionImageById(imageID:number){
    console.log("To delete image with id: "+imageID);
    //this.router.navigate(['remove', id]);
    this.userService.deleteImageById(imageID)
      .subscribe(data =>
      { this.router.navigate(['touristAttraction/edit/', this.touristAttractionid]);
        location.reload()});
